 ...
 */
 ...
 export function findPath(graph: Graph, start: string, end: string): string[] {
     const queue = [ start ]
     const visited: { [vertex: string]: boolean } = { [start]: true }
     const depth: { [vertex: string]: number } = { [start]: 0 }
 
     while (queue.length > 0) {
         const currVertex: string = queue.shift()!
         for (const adjVertex of graph[currVertex]) {
